...
export default function UserStatusGrid({ filter, searchValue }) {
    ...
    const userData = useMemo(() => {
        ...
        if (filter) {
            if (filter === "new") {
                const sortedData = [...requestData.results].sort((a, b) =>{ 
                    console.log("result of date",new Date(b.request_date) - new Date(a.request_date))
        ...
        <>
            ...
            >
                {userData && userData.map((user, index) => {
                    if(user.status === 'อ่านแล้ว' || user.status === 'ยังไม่อ่าน'){
                        return(
                            <Box key={index}>
                            <Card style={{ background: "#D7C0FD", width: "96%", marginTop: 8, marginLeft: 10 }} onClick={() => {
                                axios.put(`http://localhost:5000/plug_mood/read-request?id=${user.request_id}&type=3`).then(() =>{
                                    navigate('/user/request', { state  : { user_id: user.user_id , image_id : user.image_id, request_id: user.request_id,}})
